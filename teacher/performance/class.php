...
switch ($_POST['form']) {
	...
	case 'displayproductlist':
		if ($_POST['srchprod'] != '') {
			$searchuseracc = "WHERE (a.productname LIKE '%" . $_POST['srchprod'] . "%' OR CASE WHEN b.middlename = '' OR b.middlename IS NULL THEN CONCAT(b.lastname, ', ', b.firstname) ELSE CONCAT(b.lastname, ', ', b.firstname, ' ', LEFT(b.middlename, '1'), '.') END LIKE '%" . $_POST['srchprod'] . "%')";
		} else {
			$searchuseracc = "";
		}
		...
		$page = $_POST["page"];
		$limit = ($page - 1) * 10;
		$res = mysqli_query($connection, "SELECT a.product_id, a.productname, a.productdesc, a.price, a.quantity, a.unit, a.availat, a.availuntil, CASE WHEN b.middlename = '' OR b.middlename IS NULL THEN CONCAT(b.lastname, ', ', b.firstname) ELSE CONCAT(b.lastname, ', ', b.firstname, ' ', LEFT(b.middlename, '1'), '.') END, a.id FROM products AS a LEFT JOIN users AS b ON a.seller_id = b.user_id " . $searchuseracc . " ORDER BY a.product_id ASC LIMIT " . $limit . ",10");
	...
	case "loadproductlistPagination":
		...
		$page = $_POST["page"];
		$rowCount = mysqli_fetch_row(mysqli_query($connection, "SELECT COUNT(a.id) FROM products AS a LEFT JOIN users AS b ON a.seller_id = b.user_id " . $searchuseracc . ";"));
		$rowsperpage = 10;
		$range = 1;
...
?> <option value="">- Select Category -</option> <?php
	                        ...
	                        <td style='white-space: nowrap;'>" . $row[7] . "</td>
																														...
																														case "loadmdlappselllistPagination":
																															$page = $_POST["page"];
																															$rowCount = mysqli_fetch_row(mysqli_query($connection, "SELECT COUNT(a.id) FROM users AS a LEFT JOIN user_details AS b ON a.user_id = b.user_id WHERE a.usertype = 'SELLER' AND a.status = 'APPROVED';"));
																															$rowsperpage = 10;
																															$range = 1;
																															$totalpages = ceil($rowCount[0] / $rowsperpage);
																															$prevpage;
               				...
               				<input type='file' class='dropify clearinfo reqresinfo dropdropdrop' id='txtproductImagefile1' name='txtproductImagefile1' accept='image/*' data-height='220' data-default-file='../" . $row[0] . "' value='" . $row[1] . "'>
               				</div><input type='hidden' class='form-control clearinfo' id='txtimageprodIDcount1' name='txtimageprodIDcount1' value='" . $row[2] . "'>|";
																																} else {
																																	echo "<div class='col-md-4 mb-2'>
               				<input type='file' class='dropify clearinfo reqresinfo dropdropdrop' id='txtproductImagefile" . $count . "' name='txtproductImagefile" . $count . "' accept='image/*' data-height='50' data-default-file='../" . $row[0] . "' value='" . $row[1] . "'>
																															...
																															$productdet = mysqli_fetch_array(mysqli_query($connection, "SELECT a.seller_id, CASE WHEN b.middlename = '' OR b.middlename IS NULL THEN CONCAT(b.lastname, ', ', b.firstname) ELSE CONCAT(b.lastname, ', ', b.firstname, ' ', LEFT(b.middlename, '1'), '.') END, a.productname, a.productdesc, a.quantity, a.unit, a.availat, a.availuntil, a.price FROM products AS a LEFT JOIN users AS b ON a.seller_id = b.user_id WHERE a.product_id = '" . $_POST['product_id'] . "';"));
