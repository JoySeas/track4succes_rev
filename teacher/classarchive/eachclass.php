            ...
            <div class="col-xs-12">
                <div class="banner-container" style="position: relative;">
                    ...
                    <div class="dropdown" style="position: absolute; top: 10px; right: 50px;">
                        <button class="btn d-flex align-items-center" type="button" id="unarchive-classroom" data-classroom-id="<?php echo $classroom_id; ?>"
                                ...
                                <?php while ($row = mysqli_fetch_assoc($result)): ?>
                                    <div class="col-12 col-md-3">
                                        <div class="card"
                                            ...
                                            <a href="./index.php?url=eachsubjectarchive&subject_id=<?php echo $row['subject_id']; ?>"
                                                ...
                                                foreach ($students as $index => $student):
                                                    ...
                                                    <li class="custom-list-item <?php echo $colorClass; ?>">
                                                        ...
                                                        <span class="delete-icon"
                                                            ...
                                                            <img src="classarchive/delete.png" alt="Delete"
                    ...
                    <div class="modal-dialog" role="document">
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                ...
                                <form action="classarchive/add_subject.php" method="POST" enctype="multipart/form-data">
    ...
    <script>
        $(document).ready(function () {
            $('#enrollStudentsForm').on('submit', function (event) {
                ...
                $.ajax({
                    url: 'classarchive/enrollstudents.php',
                    type: 'POST',
                    data: formData,
                    contentType: false,
                    processData: false,
        ...
        function confirmDeletion(studentId) {
            Swal.fire({
                title: "Are you sure you want to unenrolled this student?",
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        url: 'classarchive/delete_student.php', // URL to your delete script
                        type: 'POST',
                        data: {
                            student_no: studentId
                        },
                        dataType: 'json', // Specify the type of data expected back
                        success: function (response) {
                            if (response.status === 'success') {
                                Swal.fire("Deleted!", "The student has been deleted successfully.", "success")
                                    .then(() => {
        ...
        $(document).ready(function () {
            ...
            $('#updateClassroomForm').on('submit', function (e) {
                ...
                $.ajax({
                    url: 'classarchive/update_classroom.php',  // Path to the PHP script that processes the update
                    type: 'POST',
                    data: formData,
            ...
            $('#delete-classroom').on('click', function () {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        $.ajax({
                            url: 'classarchive/delete_classroom.php', // The server-side script to handle deletion
                            type: 'POST',
        ...
        $(document).ready(function () {
    $('#unarchive-classroom').on('click', function (e) {
        ...
        Swal.fire({
            title: 'Are you sure?',
            text: "This classroom will be unarchived!",
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#3085d6',
            cancelButtonColor: '#d33',
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: 'classarchive/unarchive_classroom.php', // Server-side script for archiving
                    type: 'POST',
                    ...
                    success: function (response) {
                        ...
                        if (jsonResponse.status === 'success') {
                            Swal.fire(
                                'Archived!',
                                'The classroom is now active, you will redirect to Classroom Tab.',
                                'success'
                            ).then(() => {
        ...
        $(document).ready(function () {
            $('#uploadRatingsForm').on('submit', function (event) {
                ...
                $.ajax({
                    url: 'classarchive/upload_ratings.php', // The PHP script for uploading ratings
                    type: 'POST',
                    data: formData,
                    contentType: false,
                    processData: false,
