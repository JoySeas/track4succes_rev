...
// Get the announcement ID from the URL, defaulting to 0 if not provided
$announcement_id = isset($_GET['id']) ? intval($_GET['id']) : 0;

// Ensure that $announcement_id is a valid integer
...
$sql = $conn->prepare("SELECT * FROM announcements WHERE id = ?");
$sql->bind_param("i", $announcement_id);
