...
// Fetch teachers_classroom from the database
$query = "SELECT c.class_name, c.section, c.description, c.class_image, t.firstname, t.lastname 
          FROM teachers_classroom c 
          INNER JOIN users t ON c.teacher_id = t.user_id 
          ORDER BY c.created_at DESC";
$result = mysqli_query($connection, $query);

// Define the image directory path
$imagePath = "http://yourdomain.com/uploads/class_images/";
...
if (mysqli_num_rows($result) > 0) {
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        $classImageFullPath = (!empty($classImage)) ? $imagePath . $classImage : 'http://yourdomain.com/uploads/classes/default-class.png';
        // Output for debugging purposes
        // echo '<p>Image Path: ' . $classImageFullPath . '</p>';
?>
...
<?php
    ...
    echo '<p>No classroom available.</p>';
}
